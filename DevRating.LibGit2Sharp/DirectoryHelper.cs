...
namespace DevRating.LibGit2Sharp
{
    ...
    public static class DirectoryHelper
    {
        ...
        private static void DeleteDirectory(string directoryPath, int maxAttempts, int initialTimeout,
            int timeoutFactor)
        {
            for (int attempt = 1; attempt <= maxAttempts; attempt++)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (!whitelist.Any(knownExceptionType => IntrospectionExtensions.GetTypeInfo(knownExceptionType)
                        .IsAssignableFrom(caughtExceptionType)))
                    {
                        throw;
                    }
