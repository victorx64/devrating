...
namespace DevRating.LibGit2SharpClient
{
    public sealed class LibGit2Diff : Diff
    {
        ...
        private readonly Envelope<IConvertible> _link;

        public LibGit2Diff(string start, string end, IRepository repository, string key, Envelope<IConvertible> link)
            : this(repository.Lookup<Commit>(start), repository.Lookup<Commit>(end), repository, key, link)
        {
        }

        public LibGit2Diff(Commit start, Commit end, IRepository repository, string key, Envelope<IConvertible> link)
            : this(start, end, new CachedHunks(new LibGit2Hunks(start, end, repository)), key, link)
        {
        }

        public LibGit2Diff(Commit start, Commit end, Hunks hunks, string key, Envelope<IConvertible> link)
